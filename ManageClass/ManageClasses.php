<?php
 ...
 if($_POST["editclass"] == 1 && $_POST["title"] != "" && $_POST["teacher"] != "" && $_POST["semester"] != "" && $_POST["roomnum"] != "" && $_POST["periodnum"] != "")
 {
  ...
  for($i=0; $i<sizeof($days); $i++)
  {
   ...
   $mytitle = htmlspecialchars($_POST[title]);
   $query = mysql_query("UPDATE `courses` SET `coursename`='$mytitle', `teacherid`='$_POST[teacher]', `semesterid`='$_POST[semester]', `sectionnum`='$_POST[sectionnum]', `roomnum`='$_POST[roomnum]', `periodnum`='$_POST[periodnum]', `dotw`='$dotw', `substituteid`='$_POST[substitute]' WHERE `courseid`='$_POST[courseid]' LIMIT 1")
